...
When the user chooses verification, set `VERIFYING` and `result_level: VERIFIED_PENDING`. Treat the affirmative choice as approval to verify the already identified matter-wide issue map using minimally necessary, de-identified search facts and the default internal ceiling of `3500 POINT`, unless the user asks for a lower ceiling. Do not recite those internal parameters in the first upgrade reply. Before the first external retrieval, give one compact transparency sentence only if it has not already been covered: “接下来会向元典发送脱敏后的检索问题，并使用账户免费额度。” Ask a new question only if scope is materially unclear, the user wants a lower limit, a new matter appears, or the next call would cross the ceiling. Before the first chargeable retrieval call, record live unit prices; if they cannot be obtained, do not make a chargeable call.
