# 基于 GPT Live 的语音交互法律咨询

一份有竞争力的法律分析，先要把事实讲清楚，再把适用法律和相似案例查明白。这个项目把自然讲述、材料阅读、法规与案例核查和持续推理放进同一个本地工作区。

![自然讲述、法规与案例核查和持续推理共同形成深入、可回查的法律分析](001_使用说明与配置/004_法律咨询工作区逻辑.png)

**语音交互帮助补足事实侧上下文，Legal MCP 提供可回查的法律侧上下文，Agent 负责持续追问、推理和反向验证。** 三者在同一个本地工作区里协作，形成一份深入、可回查的法律分析。

本项目把这套逻辑做成了一个面向中国大陆法律问题的 Codex 本地工作包，串联 GPT Live 语音对话、材料阅读、公开案例参考、现行法与案例数据库核查和报告生成。用户不需要先写出完整案情，也不用判断哪些细节“有法律意义”：可以先自然地讲，Agent 每次追问一个最影响判断的问题，主动邀请提供关键证据，并在必要时用公开案例解释为什么这个细节重要。初步判断不要求预先配置检索工具；需要更准确时，再核查现行法规、案例全文和相反观点。
...
```text
公开信息辅助的初步判断 → 法律数据库精准核查 → 必要时选择真人律师复核
```

1. **初步判断**：通过语音或文字了解情况，逐步确认目标、时间线、关键证据、矛盾、对方可能的说法和仍会改变判断的未知项。必要时检索法院公开案例或可靠报道，说明大致裁判方向以及为什么某个问题重要。公开资料可能不完整或不是最新；如果已经够用，可以直接停止。
2. **法规与案例数据库核查**：如果你希望更准确，Agent 会带你配置免费的元典 Key，再核对当前规则、裁判全文、支持与相反观点及案例覆盖面。精读或引用的裁判全文会保存在本地，并升级同一份咨询结果。
3. **真人复核**：如果事项重大、准备采取不可逆行动，或者核查后仍有困惑，可将项目打包发送给律师进行复核，不必再复述案件。是否联系、接受委托、收费和开始复核，仍以你与律师后续确认的信息为准。

## 你会得到什么

- 一份持续更新的 `004_给你的结果/001_咨询结果.md`，清楚区分用户陈述、证据状态、公开信息参考、数据库核查、条件性判断和待核查问题；
- 保存在本地的材料清单、访谈记录、工作底稿和处理副本，暂停后可以继续，不必重新讲述；
- 精读或引用的裁判全文 Markdown 文件和持续检索记录，报告直接链接本地原文；独立索引按需提供；
- 需要时生成的行动清单和律师复核委托说明。
